---
title: "microsoft/ai-agents-for-beginners"
created: 2026-05-18
updated: 2026-05-20
type: tutorial
tags: [python, ai-agent, tutorial, rising]
sources: [raw/trending/2026-05-20.json]
confidence: high
trending_count_daily: 4
trending_count_weekly: 0
trending_count_monthly: 0
consecutive_days: 4
first_trending: 2026-05-18
last_trending: 2026-05-20
peak_rank: 12
total_stars: 63453
language: Jupyter Notebook
---

# microsoft/ai-agents-for-beginners

## 概述

微软出品的 AI Agent 入门教程，12 节课程带你从零开始构建 AI Agent。

- **Language:** Jupyter Notebook
- **Stars today:** +1012
- **Total stars:** 63,453
- **GitHub:** [microsoft/ai-agents-for-beginners](https://github.com/microsoft/ai-agents-for-beginners)
- **Rank:** #13 on GitHub Trending (2026-05-19)

## 同类项目

- [[humanlayer-12-factor-agents]] — 同为 AI Agent 方法论/原则
- [[K-Dense-AI-scientific-agent-skills]] — Agent 技能实践
