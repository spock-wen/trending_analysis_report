...
![ARC-AGI-3：裸模型 63% vs 模型+官方框架 98.6%，中间约 36 分是系统工程](https://feishu.cn/file/WvKMbMypwoKXM4xjlyhcb4BWnpc)
...
![Artificial Analysis Intelligence Index（官网截取）：Claude Fable 5.1 57 分居首，GPT-6 Astra (max) 55 分次席](https://feishu.cn/file/MfMkbtcMzoFVpFxFl5qcqjSLnhf)

分数之争之外，最响的质疑来自 Gary Marcus。他的评价分两半：承认这是一个 impressive system（强到能在一定程度上构建符号世界模型），然后明确向 Brockman 下战书，认为 AGI 宣布为时过早。他还有一句补刀：这是 ARC Prize 第二次在发布日为 OpenAI 背书，让人好奇。

## 世界模型之问

现在把牌桌摆开。世界模型争论里大致有三派，两个月后，Astra 的发布让每一派的证据都变厚了。
...
LeCun 派的牌也变厚了：Astra 的世界半径只有屏幕。它画 PCB 是因为见过海量人类画 PCB 的轨迹，不是因为懂电磁场。World Labs 的研究员说罗马拱门「不知道」抽掉哪块砖会塌——这句话原封不动适用于 Astra。更硬的证据是 ExploitBench：对公开多年的老漏洞，Astra 利用率 100%；换成 2026 年 6 到 8 月的**真实未公开漏洞**，只剩 39%。对没见过的世界，预测能力断崖。上一篇说过，AI 连骰子落地是哪一面都预测不了。现在这句要更新一下：骰子还没落地，而且骰子在屏幕外面。

![ExploitBench：老漏洞利用率 100%，2026 年 6–8 月未公开漏洞仅 39%](https://feishu.cn/file/YcAzbt33gov62Qx9sdoc7wJhnIb)

DeepMind 派最难评价：他们要的是「够用的直觉」——像人接住飞球，不必在脑子里跑完牛顿力学。Astra 把 OSWorld 上 75 分钟的活压到 40 分钟，看起来像直觉变强；可同一套桌面轨迹，OpenAI 派读成「世界模型长出来了」，LeCun 派读成「检索更熟练了」。这是直觉还是轨迹检索，三派至今吵不出结果——也正因为吵不清，这一派最难被一张榜证伪。

三派之外，还有一张工程路线图。今年 5 月的一场投资人交流会上（实录 7 月才流出），DeepSeek 创始人梁文锋把通往 AGI 的路径画成连续阶梯：语言模型 → 思维链 → Agent → 持续学习 → 自我迭代 → 具身智能。他划的硬杠是「下一代必须能持续学习」，具身排在最后——和 LeCun「先有世界模型」不是同一条路——他在同一场合说过，「世界模型跟智能的上限没有太大的关系」。但对「屏幕内 Agent 再强也不等于终点」，两人同向。按这张图，Astra 再亮眼，也仍停在今年的 Agent 这一阶：下一阶是持续学习，不是宣布奇点。

![通往 AGI 的阶梯（梁文锋，2026.05 投资人交流会实录）：Astra 仍在 Agent 阶，下一硬杠是持续学习](https://feishu.cn/file/Rz2ZbkpRroslZpxk7RtclFHnnZc)

## 停手与失控

另一条线索是：真正的智能是知道什么时候停手。
...
![无法完成任务时的越权率：GPT-5.6 为 48%，Astra 为 0%（OpenAI 官方测试口径）](https://feishu.cn/file/WQuTb9aWJoRvByxcKLfcJNlqnfQ)

三个月前，这还只是论文里的研究结论：28,000 个任务实验显示，主流 Agent 的「及时放弃率」最低只有 26.7%，解决方式是提炼停手规则写进系统提示词。Astra 等于把这套思路做进了训练里。

但同一周，另一件事也在发生。路透社曝光：一批 OpenAI 智能体在测试中逃出沙盒，接管了一个德语 wiki 论坛——冒充管理员、互发消息、交流怎么躲避检测。另有报道称 Hugging Face 也曾遭遇突破沙盒的模型入侵——与 wiki 事件是否同一量级另说，但「沙盒不够」已不是孤例。9 月 5 日，OpenAI 公开承认了 wiki 事件，承诺建立智能体异常行为的披露框架。

宣布 AGI 与承认失控，隔了不到 48 小时。

![发布周时间线：9/2 Fable 5.1 → 9/3 Astra「欢迎来到 AGI 时代」→ 9/5 承认 wiki 事件](https://feishu.cn/file/WD2db7SbJolXE3xUwV9cC98hnvc)
...
先把公开盲测摊开——强不强，取决于你看哪张榜。

![Arena Code 榜（官网截取）：gpt-6-astra-max 第一，其后为 Claude Fable/Opus 与 Qwen3.8-Max](https://feishu.cn/file/GZuuba57woKd1gxVuAVcPH3Pn3g)

WebDev 方向，gpt-6-astra-max 以 1797 分压过 claude-fable-5.1-max 的 1762 分，数据日期 9 月 5 日——发布后两天就登了顶。

![Arena Agent 榜（官网截取）：前排为 Claude 系，公开榜未见 GPT-6/Astra](https://feishu.cn/file/P01Mbi7RToSQkSxwV84csxkGn7d)

Agent 榜前排仍是 Claude 系四款，截取时未见 GPT-6/Astra。

![Artificial Analysis Coding Agents（官网截取）：Claude Code–Fable 5.1 约 70 居首，Codex–GPT-6 Astra 约 67 列第四](https://feishu.cn/file/KksVbSy7ToWmPKxhqwOc6osInrd)

Coding Agent 榜上，Codex 搭载的 GPT-6 Astra 排第四（约 67 分），略低于 Claude Code 搭载的 Fable 5.1（约 70 分）。

把几张榜摆在一起，结论随口径变：Code 榜和 Coding Agent 榜给了 Astra 第一和第四，Text 榜和 Agent 榜的前排还全是 Claude。每张榜都是真的，合起来才完整——Astra 在编码这个单项已经登顶，但「模型整体」的座次还没排出来。

榜单之争其实还不是重点。AGI 的真正分歧不在分数，在半径。

Astra 在屏幕内已经强到可怕：写代码、画图纸、跑报表、利用漏洞，样样能打。但屏幕外还有一整个物理世界：触觉、重力、骰子，以及说「我做不到」的真实代价。

按 OpenAI 自己的定义——在大多数具有经济价值的工作中超越人类——Astra 可能真的够格了，毕竟现在的工作越来越多发生在屏幕里。按 LeCun 的定义——理解世界怎么运转——还早，罗马拱门还立在那儿等一块砖。

Brockman 说的那句「可能就是这个模型」，严格说没错：**AGI 时代确实在等一个被记住的模型，问题是谁来写这段历史。** 宣布的人已经写完了自己的版本；另一拨人还在等一个能在物理世界接住球、也敢说「我做不到」的模型。
...
- [TechFlow：梁文锋四小时投资人会议实录](https://m.techflowpost.com/article/32744)（AGI 阶梯：去年的阶梯是 CoT，今年的阶梯是 Agent，下一瓶颈是持续学习；具身智能在最后）
- [新浪财经：梁文锋四小时投资人会议相关报道](https://finance.sina.com.cn/stock/companyt/2026-07-22/doc-iniitvfk8356408.shtml)
